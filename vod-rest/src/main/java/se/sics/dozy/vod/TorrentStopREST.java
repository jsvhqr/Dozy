...
public class TorrentStopREST implements DozyResource {
    ...
    public Response pendingUpload(FileInfoJSON fileInfo) {
        ...
        TorrentStopEvent.Request request = new TorrentStopEvent.Request(fileInfo.getName(), new IntIdentifier(fileInfo.getOverlayId()));
        try {
            ...
            DozyResult<TorrentStopEvent.Response> result = futureResult.get();
            ...
            LOG.error("stop:{}<{}> status:{}", new Object[]{request.eventId, request.fileName, Response.Status.INTERNAL_SERVER_ERROR});
